...
The RoboRacer Sim Racing League is a virtual competition that complements the physical RoboRacer Autonomous Racing Competition. It utilizes the [AutoDRIVE Ecosystem](https://autodrive-ecosystem.github.io) to simulate a digital twin of a RoboRacer vehicle on a virtual racetrack. The primary goal is to make autonomous racing accessible globally.
...
*   **Qualification Race:** Teams must demonstrate their ability to complete multiple laps around a practice track without colliding with track boundaries.
...
*   **Qualification Round:** Teams complete 10 laps around the provided practice track.
*   **Qualification Results Declared:** Standings of qualified teams are released.
*   **Competition Track Released:** Organizers release the official competition track.
...
This competition follows a **time-attack racing** format where each team competes independently on the same racetrack. Each race consists of 12 laps: one warm-up lap, 10 race laps, and one cool-down lap.
...
*   **Competition:** In this phase, teams aim to complete 10 consecutive autonomous laps as quickly as possible without exceeding the collision tolerance. Exceeding the collision tolerance leads to disqualification. Teams are ranked by their total time to complete 10 race laps.

### 2.2. Competition Terminology

*   **Collision:** Any contact between the vehicle's colliders and the racetrack bounds (excluding wheels on the ground) is a collision. Each collision incurs a 10-second penalty (cumulative: 10s for the first, 20s for the second, etc.). More than 10 collisions result in disqualification. Collisions reset the vehicle to the last checkpoint, and the lap timer does not reset.
*   **Warm-Up Lap:** The first lap. Time and collisions are not considered. This lap allows algorithms to initialize and connect with the simulator.
*   **Race Laps:** The 10 laps following the warm-up lap. Time and collisions during these laps are considered.
*   **Cool-Down Lap:** The final lap. Time and collisions are not considered. This lap is optional but can be used to showcase skills.
*   **Checkpoints:** Virtual markers along the track, including the start/finish line. Their exact locations are not revealed. Passing all checkpoints is necessary for a valid lap time.
*   **Lap Time:** The time taken to complete one full lap. The timer starts when the previous lap ends and stops when the current lap ends.
*   **Race Time:** The cumulative time to complete 10 race laps. The timer starts after the warm-up lap ends and stops after the 10th race lap. Collision penalties are added separately.
*   **Best Lap Time:** The fastest lap time among the 10 race laps.
...
*   **Inspection:** Race stewards inspect team submissions.
*   **Simulator:** The AutoDRIVE Simulator container is launched with specific settings (e.g., "Ultra" graphics).
*   **Devkit:** The AutoDRIVE Devkit container with the team's algorithm is launched. A ROS 2 bag recording starts.
...
*   **Race:** The race includes a warm-up lap, 10 race laps, and a cool-down lap. Only race lap times and collisions are considered.
*   **Data:** Performance metrics, screen recordings, and ROS 2 bags are stored.

### 2.4. Inspection Rules

**Warning:** Organizers reserve the right to reject submissions that exploit the competition framework unethically. All submissions are inspected before the race.

**Warning:** Teams violating the code of conduct will be disqualified and publicly cited for "malpractice."

Modifications to the competition framework (simulator, vehicle, environment, communication interface, devkit, or containerization) are strictly prohibited and will result in disqualification.

Autonomous racing algorithms using raw perception data for control commands are permissible (e.g., reactive planning, deep learning). However, using simulation ground truth data or controlling aspects other than vehicle actuators is forbidden. Unethical exploitation of the framework (e.g., back-end access, frame-grabbing, tampering with data) is considered serious malpractice.

**Info:** Refer to the [Technical Guide](https://autodrive-ecosystem.github.io/competitions/roboracer-sim-racing-guide-2024) for details on permissible and restricted data streams.

Ethical integrity and adherence to the code of conduct are expected. Malpractice or plagiarism may result in warnings, public citations, or disqualification.

### 2.5. Evaluation Criteria

*   The primary evaluation criterion is **total race time.** Tie-breakers may use best lap time or other metrics.
*   Collisions incur a **penalty** of 10 seconds per collision.
*   A maximum of 10 collisions are permissible per race; exceeding this limit results in **disqualification.**
...
**Note:** Upon running the submitted container, all necessary nodes should start automatically. After clicking the "Connection Button" in the AutoDRIVE Simulator, the simulated vehicle should begin running. Ensure all necessary commands (sourcing workspaces, setting environment variables, launching nodes) are within the `entrypoint` script (`autodrive_devkit.sh`) of the `autodrive_devkit` container. Do **NOT** use `~/.bashrc` or similar methods for automatic execution. Organizers must be able to start additional bash sessions within the container for data recording and inspection without automatically executing your codebase.

### 3.1. Submission Requirements

*   Submit a Docker container image of the autonomous racing software stack.
*   The container must be self-sustainable with all dependencies and configured for automatic command execution.
*   The container must be based on the official AutoDRIVE Devkit container.
*   Teams may add/modify ROS 2 packages within the AutoDRIVE Devkit container for their algorithms. Modifications to existing container elements are prohibited.
*   The simulator container does not need to be submitted.
...
*   **Containers:** Push Docker images to private [DockerHub](https://hub.docker.com) repositories.
*   **Data:** Store data in encrypted cloud storage or locally with backups.

After the competition, teams are encouraged to publish their source code and Docker containers under open-source licenses and make race data publicly available to enhance visibility and future competitions.

## 4. Citation

Please cite the following papers if you use any part of the competition framework:
