...
The knockout races will then determine the winners, with the fastest lap deciding each matchup. In case of a tie, the best lap time from the open track trials will be used.
  ...
  3. To increase the quality of the future Roboracer competitions, the top 3 teams of each race are encourage to publish the code of their algorithm under an open-source license in the [Roboracer repository](https://github.com/f1tenth) on Github.

  4. In order to better accomodate all participating teams, all teams should have **at most 10 team members** present at the race space (includes sideline and seating area) during the event. There are no restrictions on the number of team members otherwise and all members will receive certificates of participation.
...
Only cars that meet the following constraints will be allowed to participate:
  ...
  9. _Tires_ : There are no resutrictions on the tires used by the car. Any and all tires that fit the wheels of the chassis are permitted.
