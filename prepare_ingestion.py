...
def main() -> int:
    ...
    skip_binary = not args.keep_binary
    ...
    if args.finetune and records:
        _, api_base, model_used, _ = resolve_finetune_config()
        tqdm.write(
            f"Finetune: {len(records)} pages, model={model_used}, base={api_base}, "
            f"concurrency={args.finetune_concurrency}",
            file=sys.stderr,
        )
        finetune_results = asyncio.run(
            run_finetune_batch(
                records,
                concurrency=args.finetune_concurrency,
                max_input_chars=args.finetune_max_input_chars,
            )
        )
    ...
    with manifest_path.open("w", encoding="utf-8") as mf:
        for rec in tqdm(records, desc="Writing output", unit="page"):
            ...
            if args.finetune and rec.stem in finetune_results:
